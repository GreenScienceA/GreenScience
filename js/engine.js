...
// ══════════════════════════════════════════════════
// SHARED NAVBAR (edit once, applies to all pages)
// ══════════════════════════════════════════════════
(function renderNav() {
  ...
  el.innerHTML = '<div class="nav-inner">' +
    ...
    '<div class="nav-right">' +
      '<button class="theme-btn" onclick="toggleTheme()" aria-label="Toggle theme">' +
        ...
        '<svg class="theme-icon-dark" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z"/></svg>' +
      '</button>' +
      '<button class="search-btn" onclick="openSearch()" title="Search (Ctrl+K)" aria-label="Search">' +
...
function renderLangBadges(item) {
  if (!item) return '';
  var langs = [];

  // Priority 1: explicit langs array (quiz-style)
  if (item.langs && Array.isArray(item.langs) && item.langs.length > 0) {
    langs = item.langs.map(function(l) { return l.toUpperCase(); });
  }
  // Priority 2: single lang string
  else if (item.lang) {
    langs = [item.lang.toUpperCase()];
  }
  // Priority 3: auto-detect from title object keys
  else if (item.title && typeof item.title === 'object') {
    if (item.title.en) langs.push('EN');
    if (item.title.fr) langs.push('FR');
    if (item.title.ar) langs.push('AR');
  }

  if (langs.length === 0) return '';

  var html = '<span class="lang-badges">';
  langs.forEach(function(l) {
    var cls = l === 'EN' ? 'lang-badge-en' : l === 'FR' ? 'lang-badge-fr' : l === 'AR' ? 'lang-badge-ar' : 'lang-badge-other';
    html += '<span class="lang-badge ' + cls + '">' + l + '</span>';
  });
  html += '</span>';
  return html;
}
...
// Sort an array of items alphabetically by their localised title
...
// ══════════════════════════════════════════════════
// PERFORMANCE: CACHED DATA COLLECTORS
// ══════════════════════════════════════════════════
// getAllBooks/getAllQuizzes are called multiple times per render.
// Cache results and invalidate on language change.

var _cachedBooks = null;
var _cachedQuizzes = null;
var _cacheLang = null;
...
function getAllBooks() {
  if (_cachedBooks && _cacheLang === currentLang) return _cachedBooks;
  var books = _collectAllBooks();
  _cachedBooks = books;
  _cacheLang = currentLang;
  return books;
}

function _collectAllBooks() {
  var books = [];
  modules.forEach(function(mod) {
...
// ══════════════════════════════════════════════════
// ALL QUIZZES COLLECTOR (for quizzes.html page)
// ══════════════════════════════════════════════════

function getAllQuizzes() {
  if (_cachedQuizzes && _cacheLang === currentLang) return _cachedQuizzes;
  var quizzes = _collectAllQuizzes();
  _cachedQuizzes = quizzes;
  return quizzes;
}

function _collectAllQuizzes() {
  var quizzes = [];
  modules.forEach(function(mod) {
    var modTitle = mod.title;
    var modId = mod.id;
...
// ══════════════════════════════════════════════════
// MODULE SUPER-CATEGORIES (for homepage filtering)
// ══════════════════════════════════════════════════

var moduleSuperCategories = {
...
var superCategoryLabels = {
  'earth-env':    'catEarthEnv',
  'life-sci':     'catLifeSci',
  'fundamental':  'catFundamental',
  'tech-comp':    'catTechComp',
  'research':     'catResearch',
};

var superCategoryIcons = {
  'earth-env':    '🌍',
  'life-sci':     '🌱',
  'fundamental':  '🔬',
  'tech-comp':    '💻',
...
var superCategoryOrder = ['earth-env', 'life-sci', 'fundamental', 'tech-comp', 'research'];


// ══════════════════════════════════════════════════
// SERVICE WORKER CLEANUP (remove old SW that caused auto-refresh)
// ══════════════════════════════════════════════════
